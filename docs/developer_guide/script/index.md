...
| 方案 | 状态 | 说明 |
|:----|:----|:----|
| [**PLand-NAPI**](NAPI) | <Badge type="warning" text="实验性" /> | PLand 的**原生绑定 API** 方案，脚本可直接持有领地对象、性能更好。但**目前仍处于早期实验阶段**，还有一些问题没有验证和解决 |
| [**PLand-LegacyRemoteCallApi**](LegacyRemoteCallApi) | <Badge type="danger" text="废弃" /> | 基于远程调用桥接的方案，已进入**维护期**（不再新增接口，但在 PLand-NAPI 正式启用前会持续维护） |
...
| 已有基于 LegacyRemoteCallApi 的旧脚本 | 可继续使用，无需迁移（NAPI 正式启用前 LRCA 会持续维护） |
| 大型项目 / 对性能要求高 | 建议直接使用 [C++ 原生开发](../QuickStart) |
...
- 了解实验性的原生绑定方案 → [PLand-NAPI](NAPI)
- 使用稳定可用的桥接方案 → [PLand-LegacyRemoteCallApi](LegacyRemoteCallApi)
- 想要最强性能 → [C++ 快速开始](../QuickStart)
